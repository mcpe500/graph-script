...
function renderElement(element: DiagramElement, values: Record<string, GSValue>, traces: Map<string, Trace>, renderEmbed: RenderEmbed, offsetX: number, offsetY: number): string {
  ...
  switch (element.type) {
    ...
    case 'circle': {
      ...
      const cx = x + w / 2;
      const cy = y + h / 2;
      svg += `<circle cx="${round(cx)}" cy="${round(cy)}" r="${round(radiusPx)}" fill="${fill}" stroke="${stroke}" stroke-width="1.8"/>`;
    ...
    case 'ellipse': {
      const cx = x + w / 2;
      const cy = y + h / 2;
      svg += `<ellipse cx="${round(cx)}" cy="${round(cy)}" rx="${round(w / 2)}" ry="${round(h / 2)}" fill="${fill}" stroke="${stroke}" stroke-width="1.8"/>`;
      if (label) svg += `<text x="${round(cx)}" y="${round(cy + textSize / 3)}" text-anchor="middle" font-size="${Math.max(14, textSize)}" font-weight="700" fill="${color}">${escapeXml(label)}</text>`;
      break;
    }
    case 'badge':
