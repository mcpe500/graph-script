...
export class Parser {
    ...
    parseFuncDeclaration() {
        ...
        while (!this.check('DEDENT') && !this.isEof()) {
            this.skipNewlines();
            if (this.check('DEDENT') || this.isEof())
                break;
    ...
    parseSubDeclaration() {
        ...
        while (!this.check('DEDENT') && !this.isEof()) {
            ...
            if (stmt.type === 'ExportStatement') {
                exports.push({ name: stmt.name, value: stmt.value });
            }
        }
        this.expect('DEDENT');
